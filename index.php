     ...
     <nav class="navbar navbar-expand-lg bg-body-tertiary">
          <div class="container-fluid">
               ...
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <a href="index.php" class="p-1"><button class="btn btn-info">All Student</button></a>
                    <form action="" method="GET">
                         <input type="text" name="q" value="<?= htmlentities($search_query) ?>" placeholder="Search by name, kana name, or passport" style="line-height: 30px; width: 200px;">
     ...
     <table class="table m-3 align-middle mb-0 bg-white">
          ...
          <tbody>
               ...
               <?php foreach ($results as $key => $value) { ?>
                    ...
                    <td scope="row"><a href="cv.php?id=<?= $value['student_id'] ?>" class="btn btn-sm btn-info" target="_blank">Detail</a>
                    </td>
                    <td scope="row"> <a href="data_edit.php?id=<?= $value['student_id'] ?>" class="btn btn-sm btn-warning" target="_blank">Edit</a>

                    </td>
                    <td scope="row">
                         <a href="delete.php?id=<?= $value['student_id']; ?>" class="btn btn-sm btn-danger" onclick="confirmDelete(event, '<?= $value['student_id']; ?>')">Delete</a>
                    </td>
                    </td>
...
<script>
     function confirmDelete(event, studentId) {
          ...
          if (!confirmation) {
               ...
               // Redirect to the delete page with the student ID if the user confirms
